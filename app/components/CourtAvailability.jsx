...
const CourtAvailability = ({court, onBooking}) => {
  ...
  const fetchAvailability = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/courts/${court._id}/availability`
      );
  ...
  const processAvailabilityData = (availabilityData) => {
    ...
    weekDates.forEach(dayInfo => {
      ...
      const existingBookings = court.bookings ? court.bookings.filter(booking => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/Courts/${court._id}/bookings`,
        bookingData
      );
  ...
  return (
      <View style={styles.container}>
        <Text style={styles.sectionTitle}>Court Availability</Text>
